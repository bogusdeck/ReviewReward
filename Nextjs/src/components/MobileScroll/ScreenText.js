...
const ScreenText = ({ screen, setCurrentImg, i }) => {
  ...
  return (
    <div className={`screen-text ${isVisible ? "text-visible" : ""}`} ref={ref}>
      <div className="screen-heading text-white subfont tracking-wider">{screen.heading}</div>
      ...
      <div className="screen-description text-white subfont mt-10">{screen.description}</div>
    </div>
  );
};
